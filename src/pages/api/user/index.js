  ...
  switch (method) {
    ...
    case 'POST': // Create new user
      ...
      var user_auth_data = await executeQuery({
        query:
          'INSERT INTO user_auth(user_id, user_ph, user_hash, user_role_id) VALUES (?,?,?,?)',
        values: [userId, ph, userHash, userRole]
      })
      if (user_auth_data.error.code == 'ER_DUP_ENTRY')
      ...
      return res
        ...
        .json({
          ...
          user: { userId, ph, firstName, lastName }
        })
    ...
    case 'PUT': // Reset password for session user
      ...
      if (!(await compare(oldPassword, oldHash[0].user_hash)))
