  ...
  switch (method) {
    case 'GET': // Get user data for slug user
      ...
      var data = await executeQuery({
        query:
          'SELECT user.* ,user_auth.user_ph, user_auth.user_role_id FROM user JOIN user_auth ON user.user_id = user_auth.user_id WHERE user_auth.user_id = ?',
        values: [slug]
      })
      if (data.length <= 0)
    ...
    case 'PUT': // Update user data for slug user
      ...
      var data = await executeQuery({
        query:
          'UPDATE user SET user_first_name = ?, user_last_name = ? WHERE user_id = ?',
